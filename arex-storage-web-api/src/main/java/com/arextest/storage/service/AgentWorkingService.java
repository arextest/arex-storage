...
public final class AgentWorkingService {
    ...
    public byte[] queryConfigFile(MockCategoryType category, AREXMocker requestType) {
        RepositoryReader<AREXMocker> repositoryReader = repositoryProviderFactory.defaultProvider();
        if (repositoryReader == null) {
            return null;
        }
        MockCategoryType categoryType = repositoryProviderFactory.findCategory(category.getName());
        if (categoryType == null) {
            return null;
        }
        return zstdJacksonSerializer.serialize(repositoryReader.queryByConfigBatchNO(categoryType, requestType));
    }
}