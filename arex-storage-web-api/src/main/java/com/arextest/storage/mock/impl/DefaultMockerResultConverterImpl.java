...
public class DefaultMockerResultConverterImpl implements MockerResultConverter {
  ...
  protected <T extends Mocker> T convertFromJar(MockCategoryType category, T mocker) {
    ...
    try {
      convertedMocker = (T) mockerConverter.mockConvert(category, objectMapper.readValue(objectMapper.writeValueAsBytes(mocker), AREXMocker.class));
    } catch (Exception e) {
  ...
  @NoArgsConstructor
  @AllArgsConstructor
  private static class MockerConvertSummary {
