...
final class AREXMockerCodecImpl implements Codec<AREXMocker> {
    ...
    private void encodeIdProperty(BsonWriter writer, AREXMocker instance, EncoderContext encoderContext, PropertyModel<String> idPropertyModel) {
    ...
    private <S> void encodeProperty(BsonWriter writer, AREXMocker instance, EncoderContext encoderContext, PropertyModel<S> propertyModel) {
        ...
        if (propertyModel.shouldSerialize(propertyValue)) {
            ...
            if (propertyValue == null) {
                ...
                try {
                    ...
                    throw new CodecConfigurationException(String.format("Failed to encode '%s'. Encoding '%s' errored with: %s", this.classModel.getName(), propertyModel.getReadName(), var6.getMessage()), var6);
                }
            }
        }
    ...
    private void decodeProperties(BsonReader reader, DecoderContext decoderContext, AREXMocker instanceCreator) {
    ...
    private void decodePropertyModel(BsonReader reader, DecoderContext decoderContext, AREXMocker instance, String name) {
        ...
        try {
            ...
            throw new CodecConfigurationException(String.format("Failed to decode '%s'. Decoding '%s' errored with: %s", this.classModel.getName(), name, var8.getMessage()), var8);
        }
